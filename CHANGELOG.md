...
- Feature: The Agent environment variable `API_DOCS_POLLING_PERIOD` lets you control the frequency at which
  the Agent will fetch API docs update from configured AmbassadorMappings. Default is `60s`. Set to
  `0` to completely turn off API docs polling from the Agent.
